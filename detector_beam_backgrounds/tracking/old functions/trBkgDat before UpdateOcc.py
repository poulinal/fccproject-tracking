...
occupancies_per_batch = np.zeros((int(eventFactor*numfiles/batches), total_number_of_layers)) #we want it to be (500/20, 14) so 14 across 25 down
occupancies_per_batch_non_normalized = np.zeros((int(eventFactor*numfiles/batches), total_number_of_layers))
...
for i in range(0, len(list_overlay)): 
    ...
    for event in reader.get("events"):
        ...
        if type == "signal":
            ...
            occupancies_per_batch_file[int(i/batches)] = occupancies_a_batch_file
