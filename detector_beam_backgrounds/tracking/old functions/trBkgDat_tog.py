...
for i in range(0, len(list_overlay)):
    ...
    if i == 0:
        ...
        #update
        dic["R"]=np.concatenate((dic["R"], np.array(list_R)))
        dic["p"]=np.concatenate((dic["p"], np.array(list_p)))
        dic["px"]=np.concatenate((dic["px"], np.array(list_px)))
        dic["py"]=np.concatenate((dic["py"], np.array(list_py)))
        dic["pz"]=np.concatenate((dic["pz"], np.array(list_pz)))
        dic["gens"]=np.concatenate((dic["gens"], np.array(list_gen_status)))
        dic["pos_ver"] = np.concatenate((dic["pos_ver"], np.array(list_pos_ver)))
        
        #from hits
        dic["hits"]=np.concatenate((dic["hits"], np.array(list_index)))
        dic["hits_pL"]=np.concatenate((dic["hits_pL"], np.array(list_hit_path_length)))
        #dic["pL"] = np.array(list_path_length)
        dic["pos_hit"] = np.concatenate((dic["pos_hit"], np.array(list_pos_hit)))
        dic["unique_mcs"] = np.concatenate((dic["unique_mcs"], unique_mcs))
        dic["superLayer"] = np.concatenate((dic["superLayer"], np.array(list_superLayer)))
        dic["layer"] = np.concatenate((dic["layer"], np.array(list_layer)))
        dic["phi"] = np.concatenate((dic["phi"], np.array(list_phi)))
        dic["stereo"] = np.concatenate((dic["stereo"], np.array(list_stereo)))
        dic["pos_z"] = np.concatenate((dic["pos_z"], np.array(list_pos_z)))
